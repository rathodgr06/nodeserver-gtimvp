...
async function invoiceToMerchant(merchant, feature_result, created_at, current_date) {
    const invoice_to_merchant = await invoiceChargesModel.getInvoiceToMerchant(merchant, current_date);
    if (checkObjectIsEmpty(invoice_to_merchant)) {
        const transaction_paydart_result = await invoiceChargesModel.getTransactionPayDartChargesData(merchant);
        const merchant_result = await invoiceChargesModel.getSubMerchantData(merchant);
        let account_fee_total = 0;
        let account_fee_type = null;

        if (merchant_result && merchant_result.sell_account_fee_type === account_type.month) {
            account_fee_total = merchant_result.sell_account_fee;
            account_fee_type = account_type.month;
        } else if (merchant_result && merchant_result.sell_account_fee_type === account_type.year) {
            const invoice_merchant = await invoiceChargesModel.checkInvoiceToMerchant(merchant, account_type.year);
...
async function invoiceToPsp(merchant, feature_set, created_at, current_date) {
    const invoice_to_merchant = await invoiceChargesModel.getInvoiceToPsp(merchant, current_date);
    if (checkObjectIsEmpty(invoice_to_merchant)) {
        const transaction_paydart_result = await invoiceChargesModel.getTransactionBuyRateAndSellRateDiff(merchant);
        const feature_result = await invoiceChargesModel.getFeatureBuyData(merchant);
        const merchant_result = await invoiceChargesModel.getSubMerchantData(merchant);
        let account_fee_total = 0;
        let account_fee_type = null;
        ...
        if (merchant_result.buy_account_fee_type === account_type.month) {
            ...
            const invoice_merchant = await invoiceChargesModel.checkInvoiceToPsp(merchant, account_type.year);
            if (checkObjectIsEmpty(invoice_merchant)) {
                account_fee_total = merchant_result.buy_account_fee;
                account_fee_type = account_type.year;
            } else {
        ...
        let total = (transaction_paydart_result.txn_total_difference + transaction_paydart_result.total_refund_difference) - (feature_result.total_set_up_fee + feature_result.total_mid_fee + account_fee_total)
