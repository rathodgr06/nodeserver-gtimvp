...
const fiserv_refund = async (req, res) => {
  try {
    ...
    let captureAmount = captured_data.amount;
    let capture_data = {
    ...
    let order_txn = {};
    let order_status = "PARTIALLY_REFUNDED";
    console.log(captureAmount, capture_data.amount);
    if (captureAmount == capture_data.amount) {
    ...
    let resp_dump = {
      ...
      type: "REFUND",
      status: (order_status === "REFUNDED" || order_status === "PARTIALLY_REFUNDED") ? "AUTHORISED" : "FAILED",
    ...
    res.status(statusCode.ok).send(Server_response.successansmsg(res_obj, "Transaction successfully REFUNDED."));
  }
  catch (error) {
