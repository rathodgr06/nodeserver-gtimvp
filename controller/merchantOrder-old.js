...
let order_id = captured_data.order_id
  try {
    ...
    const _terminalcred = {
      MID: _getmid.MID,
      password: _getmid.password,
      baseurl: mode=='test'?credientials[_pspid.credentials_key].test_url:credientials[_pspid.credentials_key].base_url,
      psp_id: _getmid.psp_id,
      name: _pspid.name,
    };
...
const telr_capture_func = async (req, res) => {
 ...
 let order_detail = await orderTransactionModel.selectOne(
  ...
  let get_partial_order_amount =
        ...
        check_amount = parseFloat(get_order_amount.amount) - (parseFloat(get_partial_order_amount.amount) + parseFloat(req.body.amount.value));
        let status = "CAPTURED";
        let txn_type = "CAPTURE";
  ...
  try {
    ...
    if (telr_capture) {
      if (telr_capture.status === "E") {
        ...
        let generate_payment_id = await helpers.make_sequential_no(mode=='test'?"TST_TXN":"TXN");
        let order_txn = {
          order_id: req.bodyString("p_order_id"),
          type: txn_type,
          txn: generate_payment_id,
...
const ni_void_func = async (req, res) => {
  ...
  try {
    ...
    if (ni_capture) {
      ...
      let generate_payment_id = mode=='live' ? await helpers.make_sequential_no("TXN"): await helpers.make_sequential_no("TST_TXN");
      let order_txn = {};
      let remark = "";
      if (
        order_details.status == "CAPTURED" ||
      ...
      if (ni_capture.state == "AUTHORISED" || ni_capture.state=='REVERSED') {
        order_txn = {
          ...
          is_voided: 1,
          txn: generate_payment_id,
          status: ni_capture?.state == "REVERSED" || ni_capture.state == "AUTHORISED" ? "AUTHORISED" : "FAILED",
          amount: captured_data?.amount,
          currency: captured_data?.currency,
          payment_id: ni_capture.reference,
          remark: remark,
...
const telr_refund_func = async (req, res) => {
 ...
 // return;
  ...
  if (
    txn_details &&
    (txn_details.type == "CAPTURE" || txn_details.type == "SALE" ||
...
const ni_refund_func = async (req, res) => {
  ...
  if (
    captured_data &&
    (captured_data.type == "CAPTURE" || captured_data.type == "SALE" ||
    ...
    try {
      ...
      if (ni_capture) {
        ...
        let generate_payment_id = mode=='test'?await helpers.make_sequential_no("TST_TXN"):await helpers.make_sequential_no("TXN");
        let order_txn = {
          order_id: req.bodyString("p_order_id"),
          type: txn_type,
          txn: generate_payment_id,
...
var MerchantOrder = {
  create: async (req, res) => {
    ...
    let ins_body = {
      ...
      other_description: order_details?.description ? order_details?.description : "",
      status: status,
      origin: "API",
      order_id: order_id,
  ...
  open_create: async (req, res) => {
    ...
    let order_id = await helpers.make_sequential_no(mode=='live'?"ORD":"TST_ORD");
    let payment_token = req.body.data.payment_token;
    let urls = req.body.data.urls;
    let mid_data = await helpers.get_mid_by_merchant_id(
      req?.credentials?.merchant_id, order_details.currency,mode
    );
    ...
    merchantOrderModel
      ...
      .then(async (result) => {
        ...
        let res_order_details = {
          ...
          amount: order_details.currency + " " + order_details.amount,
          payment_link:
            process.env.PAYMENT_URL + "initiate/" + order_id + "/" + token+'/'+mode,
          iframe_link:
            process.env.PAYMENT_URL +
            "initiate/" +
            order_id +
    ...
    merchantOrderModel
      ...
      .then(async (result) => {
        ...
        if(req.body?.retry==1){
          ...
          let fetchLastTryData = await helpers.fetchLastTryData({order_id:req.bodyString('order_id'),mode:req.bodyString('mode')});
        ...
        merchantOrderModel
          ...
          .then(async (result_1) => {
            ...
            data.order_details['is20minExceed'] =  checktime20min.length  === 0  ? 1 : 0;
  ...
  get_details: async (req, res) => {
    ...
    merchantOrderModel
      ...
      .then(async (result) => {
        ...
        let selection =
          "order_id,customer_name as name,customer_email as email,customer_mobile as mobile,,customer_code as code,amount,currency,status,return_url,payment_token_id as payment_token";
        merchantOrderModel
          .selectOne(
            selection,
            {
  ...
  pay: async (req, res) => {
    ...
    let generate_payment_id = uid();
    let payment_id = await helpers.make_sequential_no(transaction_mode=='test'?"TST_TXN":"TXN");
    ...
    merchantOrderModel
      ...
      .then(async (result) => {
        ...
        const _terminalcred = {
          MID: _getmid.MID,
          password: _getmid.password,
          baseurl: transaction_mode=='test'?credientials.ni.test_url:credientials.ni.base_url,
          psp_id: _getmid.psp_id,
          name: _pspid.name,
        };
        ...
        try {
          ...
          let checkForCardProxyInSystem = await helpers.fetchLastTryData({card_proxy:card_proxy});
          ...
          // }
          if(_getmid.is3DS == 1  && checkForCardProxyInSystem?.['3ds_version']==0){
         ...
         helpers.updateOrderCycle(res_order_data?.order_id, transaction_mode)
          ...
          let order_txn = {
            ...
            remark: error?.response?.data?.errors?.[0]?.message != undefined ? error?.response?.data?.errors?.[0]?.message : response_category_failed?.response_details,
            txn: payment_id,
            type:
          ...
          let temp_card_details = await helpers.fetchTempLastCard({order_id:res_order_data.order_id,mode:transaction_mode});
        ...
        if (ni_order_sale.state == "AUTHORISED" || ni_order_sale.state == "CAPTURED"){
            // Add 3DS code here    
        ...
        await helpers.addTransactionFailedLogs(txnFailedLog);
          ...
          let p_request_id = await helpers.make_sequential_no(transaction_mode=='live'?"REQ":"TST_REQ");
          ...
          await helpers.common_add(order_req,transaction_mode=='test'? "test_generate_request_id":"generate_request_id");
          ...
          if (ni_order_sale && ni_order_sale.state != "FAILED") {
            ...
            let orderupdate = {
              ...
              saved_card_for_recurring: JSON.stringify(ni_order_sale?.savedCard)
          ...
          if (
            ...
            if (ni_order_sale?.["3ds2"]) {
              let three_ds2_url =
                ni_order_sale?._links?.["cnp:3ds2-authentication"]?.href || ni_order_sale?._links?.['cnp:3ds2-challenge-response']?.href;
              let request_data_3ds2 = {
                ...
                notificationURL:
                  ...
                  res_order_data.order_id+'&mode='+transaction_mode,
                browserInfo: {
                  ...
                  browserScreenHeight: "593",
                  browserScreenWidth: "517",
                  browserJavascriptEnabled: true,
                  browserIP: "106.216.245.13",
              ...
              if (ds_3_data.state == "CAPTURED" || ds_3_data.state == "AUTHORISED"|| ds_3_data.state == "PURCHASED") {
                is3ds = false;
                  /* START */
                  let p_request_id = await helpers.make_sequential_no(transaction_mode=='live'?"REQ":"TST_REQ");
                  let merchant_id = await helpers.get_data_list(
              ...
              if (ds_3_data.state == "FAILED") {
                ...
                if(transaction_mode=='live'){
                  ...
                  await merchantOrder.addDynamic(order_txn_update, "test_order_txn");
                }
                  ...
                  await merchantOrderModel.updateDynamic(
                    {
                      "3ds2_url":
                        ni_order_sale?._links?.["cnp:3ds2-challenge-response"]
                          ?.href || ni_order_sale?._links?.['cnp:3ds2-challenge-response']?.href,
                    },
                    { order_id: res_order_data.order_id },
                    order_table
                  );
          ...
          ee.once("ping", async (arguments) => {
            ...
            if (
              ...
              let merchant_and_customer_transaction_response = await qb
                .select(
          ...
          if ((ni_order_sale.state == "AUTHORISED" ||
              ni_order_sale.state == "CAPTURED") && fraudStatus == true){
              let res_obj = await voidNi(req,ni_order_sale,res_order_data,payment_id,transaction_mode,fraudStatus,browser_token_enc,p_request_id);
              return res.status(statusCode.badRequest).send(response.errorMsgWithData(res_obj.message,res_obj));
          }
  ...
  telr_pay: async (req, res) => {
    ...
    let payment_id = await helpers.make_sequential_no(req.body.env=='test'?"TST_TXN":"TXN");
    let table_name = "orders";
    let txn_table = "order_txn"
    ...
    try {
      ...
      let status = "PENDING";
  let result =  await merchantOrderModel
        .updateDynamic(
          order_data,
          {
            order_id: req.bodyString("order_id"),
          ...
          const _terminalcred = {
            MID: _getmid.MID,
            password: _getmid.password,
            baseurl:req.body.env=='test'? credientials.telr.test_url:credientials.telr.base_url,
            psp_id: _getmid.psp_id,
            name: _pspid.name,
          };
         ...
         if(telr_session?.enrolled!='Y' && telr_session?.enrolled!='N' && telr_session?.enrolled!='U' && telr_session?.enrolled!=''){
          let order_update_failed = {
            status: "FAILED",
            psp: "TELR",
            payment_id: payment_id,
          ...
          if (telr_session.enrolled == "N" || (telr_session.enrolled=='Y' && telr_session.level==2) || telr_session.enrolled=='') {
            status = "PENDING";
            ...
            if (!_getmid) {
              res
                .status(statusCode.badRequest)
                .send(
                  response.errormsg("No Routes  Available for Transection")
                );
            }
            const _pspid = await merchantOrderModel.selectOne(
              "*",
            ...
            if (_getmid.is3DS == 1) {
              ...
              let temp_card_details = await helpers.fetchTempLastCard({order_id:res_order_data?.order_id,mode:req.body.env});
                let txnFailedLog = {
                  order_id: res_order_data.order_id,
                  terminal:res_order_data?.terminal_id,
              ...
              let payment_status =
                sale_api_res.status === "A" ||  sale_api_res.status==='H' ? "CAPTURED" : "FAILED";
              ...
              if (sale_api_res.status === "A" ||  sale_api_res.status==='H') {
                ...
                if(req.body.env=='test'){
                  const txn_result = await orderTransactionModel.test_txn_add(order_txn);
                }else{
                ...
                let p_request_id = await helpers.make_sequential_no(req.body.env=='test'?"TST_REQ":"REQ");
                let merchant_id = await helpers.get_data_list(
                  "merchant_id",
                  table_name,
                  { order_id: req.body.order_id }
                ...
                await helpers.common_add(order_req,req.body.env=='test'?"test_generate_request_id":"generate_request_id");
                let res_order_data1 = await merchantOrderModel.selectOne(
                  "psp,payment_mode",
                  {
                    order_id: req.bodyString("order_id"),
                ...
                let new_res = {
                  ...
                  transaction_id: payment_id,
                  status: sale_api_res.status === "A" || sale_api_res.status === "H" ? "SUCCESS" : "FAILED",
                  status_code: "00",
        ...
        await helpers.addTransactionFailedLogs(txnFailedLog);
                ...
                return res
                  .status(statusCode.ok)
                  .send(
                    response.errorMsgWithData(sale_api_res.message, res_obj)
                  );
              }
            }
          } else {
            ...
            return res
              .status(statusCode.ok)
              .send(response.successansmsg(res_obj));
          }
     
    } catch (error) {
      let res_order_data = await merchantOrderModel.selectDynamicONE('*',{order_id:req.bodyString('order_id')},table_name);
      let browser_token_enc = req.browser_fingerprint;
  ...
  saveCard: async (req, res, next) => {
    ...
    if (req.bodyString("card_id") == "") {
      ...
      let card_details = await merchantOrderModel.selectDynamicONE('*',{id:card_id},'customers_cards');
    ...
    merchantOrderModel
      ...
      .then(async (result) => {
        ...
        new_res = {
          ...
          m_payment_token: "",
          transaction_time: moment().format("DD-MM-YYYY hh:mm:ss"),
          return_url: res_order_data.cancel_url||process.env.DEFAULT_CANCEL_URL,
          payment_method_data: {
            scheme: "",
            card_country: "",
            card_type: "",
  ...
  create_qr_order: async (req, res) => {
    ...
    let order_id = await helpers.make_sequential_no(mode=='test'?'TST_ORD':"ORD");
    let status = "PENDING";
    let token_payload = {
      order_id: order_id,
      amount: order_details?.amount,
    ...
    let ins_body = {
      ...
      currency: order_details?.currency,
      return_url: order_details?.return_url,
      description: order_details?.description == "" ? "Payment of " + order_details?.currency+ " " + total_amount : order_details?.description,
      other_description: order_details?.description,
    ...
    merchantOrderModel
      ...
      .then(async (result) => {
       /*  logs.push(
        ...
        let payment_link = mode=='test'? process.env.PAYMENT_URL + "initiate/" + order_id + "/" + token+'/test':process.env.PAYMENT_URL + "initiate/" + order_id + "/" + token+'/live';
        let res_order_details = {
          status: status,
          message: "Order created",
          token: token,
  ...
  update_3ds: async (req, res, next) => {
    ...
    const ni_sale_request = await ni_sale.updatePayment(url,req.body,_terminalcred);
    ...
    if (res_order_data && res_order_data?.status == "AWAIT_3DS") {
      ...
      await merchantOrderModel
        ...
        .then(async (result) => {
          // request id table entry
          let p_request_id = await helpers.make_sequential_no(transaction_mode=='test'?"TST_REQ":"REQ");
          let merchant_id = await helpers.get_data_list(
            "merchant_id",
            order_table,
            { order_id: res_order_data.order_id }
  ...
  update_3ds2: async (req, res, next) => {
    ...
    const _terminalcred = {
      MID: _getmid.MID,
      password: _getmid.password,
      baseurl: req.body.mode=='test'?credientials.ni.test_url:credientials.ni.base_url  ,
      psp_id: _getmid.psp_id,
      name: _pspid.name,
    };
  ...
  update_3ds2_ni: async (req, res, next) => {
    ...
    let logs = transaction_mode=='test'?await order_logs.get_test_log_data(res_order_data?.order_id):await order_logs.get_log_data(res_order_data?.order_id);
    logs.push(
      `${moment().format(
        "DD/MM/YYYY HH:mm:ss.SSS"
      )} : initiated MerchantOrder.update_3ds`
    ...
    if (res_order_data && res_order_data?.status == "AWAIT_3DS") {
      ...
      await merchantOrderModel
        ...
        .then(async (result) => {
          ...
          await order_logs.update_logs_data({ order_id: res_order_data.order_id, }, logs_payload)
          // web  hook starting
          let hook_info = await helpers.get_data_list("*", "webhook_settings", {
            merchant_id: res_order_data?.merchant_id,
          });
          ...
          if ((_nistatus == "AUTHORISED" || _nistatus == "CAPTURED")  && res_order_data.origin == "SUBSCRIPTION" && (ni_sale_request?.savedCard == "" || ni_sale_request?.savedCard == undefined)) {
          ...
          if((_nistatus == "AUTHORISED" || _nistatus == "CAPTURED" ) && fraudStatus === true && void_transaction==false){
            response_category = await helpers.get_error_category( '143', "paydart", "FAILED"  ) ;
        ...
        if(void_transaction){
          ...
          const voidTransaction = await fraudService.voidTransaction(VoidTransactionPayload);
          ...
          if (voidTransaction.status === 'success') {
           // const payment_id = await helpers.make_sequential_no(transaction_mode == 'test' ? 'TST_TXN' : "TXN");
            ...
            await merchantOrderModel.updateDynamic({
              ...
              remark: response_category.response_details,
            }, { order_id: order_id, txn: voidTransaction.data.transaction_id }, txn_table)
         ...
         return res.status(statusCode.ok).send(successdatamsg( res_obj,res_obj.message));
  ...
  tokenSummary: async (req, res) => {
    ...
    merchantOrderModel
      ...
      .then(async (result) => {
        ...
        let send_res = [];
        for (let row of result) {
          let decoded_token = JSON.parse(enc_dec.cjs_decrypt(row.browser_token));
          let customer_id = enc_dec.cjs_decrypt(row.cid);
          let last_failed_test_transaction = await helpers.last_transactions_using_token(row.browser_token, "FAILED",'test_orders');
          let last_failed_live_transaction = await helpers.last_transactions_using_token(row.browser_token, "FAILED",'orders');
          let last_successfull_test_transaction = await helpers.last_transactions_using_token(row.browser_token, ['CAPTURED','AUTHORISED','REFUNEDED'],'test_orders');
          let last_successfull_live_transaction =  await helpers.last_transactions_using_token(row.browser_token, ['CAPTURED','AUTHORISED','REFUNEDED'],'orders');
          let attempts_live = await helpers.get_orders_count(row.browser_token,'orders');
          let attempts_test = await helpers.get_orders_count(row.browser_token,'test_orders');
          let temp ={
            browser_token:row.browser_token,
            browser_fingerprint:decoded_token.browser_fingerprint,
            browser:decoded_token.browser,
            os:decoded_token.os,
            customer_details:await merchantOrderModel.selectDynamicONE('name,email,dial_code,mobile_no',{id:customer_id},'customers'),
            last_failed_live_transaction:last_failed_live_transaction?moment(last_failed_live_transaction).format('DD-MM-YYYY HH:mm'):'NA',
            last_failed_test_transaction:last_failed_test_transaction?moment(last_failed_test_transaction).format('DD-MM-YYYY HH:mm'):'NA',
            last_successfull_live_transaction:last_successfull_live_transaction?moment(last_successfull_live_transaction).format('DD-MM-YYYY HH:mm'):'NA',
            last_successfull_test_transaction:last_successfull_test_transaction?moment(last_successfull_test_transaction).format('DD-MM-YYYY HH:mm'):'NA',
            last_attempt:row.recent_used?moment(row.recent_used).format('DD-MM-YYYY HH:mm'):'NA',
  ...
  cardsByToken:async(req,res)=>{
    ...
    let results = await merchantOrderModel.selectDynamic('name_on_card,card_expiry,card_nw,last_4_digit,recent_used,email',{browser_token:browser_token,deleted:0},'customers_cards');
    res
    .status(statusCode.ok)
    .send(
      response.successdatamsg(
  ...
  telr_update_3ds_updated: async (req, res, next) => {
    ...
    try {
      const txn = await helpers.make_sequential_no(req.body.mode=='test'?'TST_TXN':'TXN');
    ...
    //   const fraudServiceRequest  = fraudService.make3dsFraudCheck(fraudCheckBody);
    //   fraudStatus = fraudServiceRequest.status === 'fail' ? true : false;
    // }
     ...
     // console.log(logs);
      ...
      if (sale_api_res.status === "A") {
        ...
        let p_request_id = await helpers.make_sequential_no(req.body.mode=='test'?"TST_REQ":"REQ");
        let merchant_id = await helpers.get_data_list("merchant_id", table_name, {
          order_id: order_details.order_id,
        });
        ...
        await helpers.common_add(order_req,req.body.mode=='test'? "test_generate_request_id": "generate_request_id");
        ...
        let temp_card_details = await helpers.fetchTempLastCard({ order_id: order_details?.order_id, mode: req.body.mode });
        let txnFailedLog = {
          order_id: order_details.order_id,
        ...
        logs.push(
          `${moment().format(
            "DD/MM/YYYY HH:mm:ss.SSS"
          )} : orderTransactionModel.addResDump with data ${JSON.stringify(response_dump)}`
        );
      ...
      if(fraudStatus === true){
          const response_category_fraud = await helpers.get_error_category( '143', "paydart", "FAILED");
          const VoidTransactionPayload = {
        ...
        const voidTransaction = await fraudService.voidTransaction(VoidTransactionPayload);
        if(voidTransaction.status === 'success'){
          const payment_id = await helpers.make_sequential_no(req.body.mode =='test'?'TST_TXN':"TXN");
          const order_txn_update = {
            txn: payment_id,
            order_id: order_details?.order_id || "",
            currency: order_details?.currency || "",
          ...
          await merchantOrderModel.updateDynamic({
            //type : res_order_data?.action.toUpperCase(),
            status :"FAILED"
          }, { order_id: order_details?.order_id }, table_name)
        }
       ...
       let temp_card_details = await helpers.fetchTempLastCard({order_id:req.body.MD,mode:req.body.mode});
        let txnFailedLog = {
          order_id: req.body.MD,
          terminal:order_details?.terminal_id,
          req:JSON.stringify(sale_payload),
  ...
  transaction_list: async (req, res) => {
    try {
      ...
      if (result.length > 0) {
        for (let val of result) {
          let trans_data = await helpers.get_trans_data(val?.order_id, req.credentials.type);
          let amount_caputred =
            await orderTransactionModel.selectCaptureAmountSum(
              "*",
              { order_id: val.order_id },
  ...
  autoCaptureTest: async (req, res) => {
    ...
    res.status(statusCode.ok).send(response.successansmsg(autoCaptureTestres,"All authorised payments captured (mode-test)"));
  },
  autoCaptureLive: async (req, res) => {
    let autoCaptureLiveres = autoCapture();
    res.status(statusCode.ok).send(response.successansmsg(autoCaptureLiveres,"All authorised live payments captured (mode-live)"));
  },
  ...
  TelrautoCaptureTest: async (req, res) => {
    ...
    res.status(statusCode.ok).send(response.successansmsg(autoCaptureTestres,"All authorised payments captured"));
  },
...
let termial_cred = await merchantOrderModel.selectDynamicONE('MID,password',{terminal_id:terminal_id},'mid');
return termial_cred;
}

let voidNi = async(req,ni_order_sale,res_order_data,payment_id,transaction_mode,fraudStatus,browser_token_enc,p_request_id)=>{
  let updated_at = moment().format("YYYY-MM-DD HH:mm:ss");
  if((ni_order_sale.state == "AUTHORISED" || ni_order_sale.state == "CAPTURED" ) && fraudStatus === true){
    let order_table = 'orders';
    ...
    const response_category = await helpers.get_error_category( '143', "paydart", "FAILED");
    const VoidTransactionPayload = {
      'order_id' : res_order_data.order_id.toString(),
      'txn_id':  payment_id.toString(),
      'action': 'VOID',
    ...
    if(voidTransaction.status === 'success'){
      ...
      await merchantOrderModel.updateDynamic({
        status :"FAILED"
      },{ order_id : res_order_data.order_id },order_table)
