...
var country = {
    ...
    list: async (req, res) => {
        ...
        if (req.bodyString("dropdown") === "yes") {
            let send_res = [];
            const countries = await CountryModel.slectDropdown(filter_arr);
    ...
    details_by_iso_code: async (req, res) => {
        let country_iso_code = req.bodyString("country_iso_code");
        CountryModel.selectOne("*", { country_code: country_iso_code, deleted: 0 })
            .then((result) => {
                let send_res = [];
                let val = result;
                let res1 = {
