...
const s2s_3ds = async (req, res) => {
    ...
    try {
        ...
        axios.request(config).then(async (final_response) => {
            ...
            const status = {
                status: (final_response.data.result === 'SUCCESS') ? (order_details.action.toUpperCase() === 'SALE'? "CAPTURED" : "AUTHORISED") : "FAILED",
                '3ds': final_response.data.authentication?.version ? 1 : 0,
                '3ds_status': final_response.data.transaction?.authenticationStatus,
            };

            await merchantOrderModel.updateDynamic(status, {order_id:order_id}, order_table);
            const order_txn = {
                txn: transaction_id.toString(),
                order_id: order_details?.order_id || "",
                currency: order_details?.currency || "",
                amount: order_details?.amount || "",
                type:  order_details.action.toUpperCase() === 'SALE'? "CAPTURE" : "AUTH",// mid_details.mode == 'SALE' ? 'CAPTURE' : "AUTH",
            ...
            let res_obj = {
                ...
                new_res: {
                    ...
                    remark: final_response?.data?.order?.authenticationStatus+'/'+final_response?.data?.response?.acquirerCode+'/'+final_response?.data?.response?.acquirerMessage+'/'+final_response?.data?.response?.gatewayCode+'/'+final_response?.data?.response?.gatewayRecommendation,//response_category?.response_details,//final_response.data.response.acquirerMessage,
            ...
            if(final_response?.data?.response?.cardSecurityCode?.gatewayCode){
                res_obj.new_res.remark = res_obj.new_res.remark+'/'+final_response?.data?.response?.cardSecurityCode?.gatewayCode;
            }
            ...
            let  redirectUrl = process.env.PAYMENT_URL+'/result/status/'+order_id+'/'+mode;
            if(order_details.success_url!=''){
    ...
    try {
        ...
        const txn_id = transaction_id?.toString() || helpers.randomTxnId();
        ...
        const insert_to_txn_table = mode == "live"
        ...
        const res_obj = {
            message: error?.response?.data?.error?.cause||"Transaction FAILED",
            order_status: "FAILED",
            payment_id: "",
            order_id: order_details?.order_id || "",
            amount: order_details?.amount || "",
            currency: order_details?.currency || "",
            remark: error?.response?.data?.explanation||"Transaction FAILED",
            new_res: {
                m_order_id: order_data?.[0]?.m_order_id || "",
                p_order_id: order_data?.[0]?.p_order_id || "",
                p_request_id: paydart_req_id.toString(),
                ...
                status: "FAILED",
                status_code: "500",
                remark: error?.response?.data?.error?.explanation||"Transaction FAILED",
                paydart_category: "FAILED",
                currency: order_details?.currency || "",
                return_url: process.env.PAYMENT_URL + "/status",
        ...
        let redirectUrl =
          process.env.PAYMENT_URL + "/result/status/" + order_id + "/" + mode;
        if (order_details.success_url != "") {
          redirectUrl = order_details?.success_url;
        }
        const inputFields = buildFormFields(res_obj);
        ...
        res.status(500).json({
            message: "Critical Failure: Unable to process transaction failure response",
