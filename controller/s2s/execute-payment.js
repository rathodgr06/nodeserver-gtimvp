...
const mtnService = require("../../service/mtnService");
const mtnSandboxService = require("../../service/mtnSandboxService");
const orangeSandboxService = require("../../service/orangeSandboxService");
...
const execuatePayment = async (req, res) => {
  try {
    ...
    if (order_id) {
      let isWallet = req.body?.paymentMethod?.is_wallet;
      console.log(`inside the execuate payment looking for is wallet`);
      console.log(isWallet);
      if (isWallet === "1") {
        let psp = req.body.paymentMethod.wallet_details.walletType;
        switch (psp) {
          case "mtn-momo":
            let mtnRes = await mtnPay(req.body, mode, order_id);
            console.log(`this is here`);
            return res.status(statusCode.ok).send(mtnRes);
            break;
          case "mtn":
            let mtnSandboxRes = await mtnSandboxPay(req.body, mode, order_id);
            return res.status(statusCode.ok).send(mtnSandboxRes);
            break;  
          case "orange-money":
            let orangeRes = await orangePay(req.body, mode, order_id);
            return res.status(statusCode.ok).send(orangeRes);
            break;
          case "orange":
            let orangeSandboxRes = await orangeSandboxPay(req.body, mode, order_id);
            return res.status(statusCode.ok).send(orangeSandboxRes);
            break;  
          case "ZEEPAY GHANA LIMITED":
          case "G-MONEY":
          case "MTN":
          case "AIRTELTIGO MONEY":
          case "VODAFONE CASH":
          case "GHANAPAY":
          case "YUP GHANA LIMITED":
            let alpayRes = await alpay(req.body,mode,order_id);
            return res.status(statusCode.ok).send(alpayRes);
            break;
        }
      } else {
        // call look up bin
        ...
        if (saveCardResponse) {
          ...
          if (lookupResult.status == "success") {
            ...
            req.card_details.card_id = enc_dec.cjs_encrypt(saveCardResponse);
            let brandingResult = await checkbrandingcard(req, order_id, mode);
            console.log(`branding result is here`);
            ...
            if (brandingResult.status == "success") {
              ...
              if (orderRoutingDetails.status == "success") {
                ...
                switch (terminalAndPSPDetails.psp_name) {
                  ...
                  case "mpgs":
                  case "mpgs_meps":
                    authenticationResult = await mpgsSessionAndAuth(
                      req,
                      order_id,
                      mode,
                      terminalAndPSPDetails.psp_name
                    );
                    break;
                }
                return res
                  .status(statusCode.ok)
                  .send(response.successansmsg(authenticationResult));
              } else {
                return res
                  .status(statusCode.badRequest)
                  .send(
                    response.errorMsgWithData(
                      `${orderRoutingDetails.message}`,
                      []
                    )
                  );
              }
              ...
              return res
                .status(statusCode.badRequest)
                .send(
                  response.errorMsgWithData(`${brandingResult.message}`, [])
                );
            }
          } else {
            return res
              .status(statusCode.badRequest)
              .send(response.errorMsgWithData(`${lookupResult.message}`, []));
          }
        } else {
          return res
            .status(statusCode.badRequest)
            .send(response.errorMsgWithData(`Something went wrong`, []));
        }
      }
    } else {
      return res
        ...
        .send(
          response.errorMsgWithData(
            `Unable to create order something went wrong`,
            []
          )
        );
    }
...
async function addOrUpdateCustomerOpenCreate(req) {
  ...
  try {
    let result = await merchantOrderModel.selectOne(
      "*",
      {
        email: email,
    ...
    if (result) {
      ...
      let resultInc = await merchantOrderModel.addDynamic(
        customer,
        "customers"
      );
      enc_customer_id = enc_dec.cjs_encrypt(resultInc.insertId);
      return enc_customer_id;
    }
  } catch (error) {
    console.log(error);
...
async function createOrder(req) {
  ...
  let ins_body = {
    ...
    billing_province: billing_details.province ? billing_details.province : "",
    ...
    shipping_country: shipping_details.country ? shipping_details.country : "",
...
async function binLookUp(req, order_id, mode) {
  try {
    ...
    if (!supportsCardCheck) {
      return {
        status: "failed",
        message: `${cardType_show} not supported by merchant`,
      };
    } else if (!DICheck) {
      return {
        status: "failed",
        message: `${DIType_show} not supported by merchant`,
      };
    } else if (!isSupportedScheme) {
      return {
        status: "failed",
        message: "Card scheme/ class not supported by merchant",
      };
    }
  ...
  try {
    ...
    if (mode === "test") {
      if (bradingDetails[0].test_card_payment_scheme) {
        const cardArr = bradingDetails[0].test_card_payment_scheme.split(",");
        ...
        if (!cardArr.includes(lookup_result.card_brand)) {
          return {
            status: "failed",
            message: "Card scheme not supported by merchant",
          };
        }
        ...
        if (
          ...
          return {
            status: "failed",
            message: "Corporate card is not supported by merchant",
          };
        }

        if (lookup_result?.is_prepaid && !cardArr.includes("PREPAID CARD")) {
          return {
            status: "failed",
            message: "Prepaid card is not supported by merchant",
          };
        }
      }
    } else {
...
async function orderrouting(req, order_id, payment_mode) {
  try {
    ...
    if (req.body.card_id && req.body.card_id !== "") {
      saved_card = true;
    }
    ...
    let isOrderAlreadyRouted = await merchantOrderModel.selectDynamicONE(
      "*",
      { order_id: order_id, mode: payment_mode },
      "order_life_cycle"
    );

    let checkForHardSoftDeclinedStatus =
      await helpers.checkForHardOrSoftDeclined({
        order_id: order_id,
        mode: payment_mode,
      });

    // if (req.body.type == "check_routing") {
    var last_transaction = await helpers.lastTwoCardUsed({
      order_id: order_id,
      mode: payment_mode,
    });
    ...
    if (isOrderAlreadyRouted?.id > 0) {
      ...
      change_card = await helpers.checkOrderWasRejected({
        status_code: ["47"],
        order_id: order_id,
        mode: payment_mode,
      });
        ...
        let new_res = {
          ...
          payment_method_data: {
            ...
            card_country: order_details?.card_country
              ...
              : "",
            card_type: order_details?.cardType ? order_details?.cardType : "",
      ...
      const routing_order = await RoutingModel.getOrderRoutingList(
        "mid_id,retry,cascade",
        {
          payment_method,
          mode: payment_mode,
          sub_merchant_id: order_details.merchant_id,
        ...
        for (const rule of rule_json_with_operator) {
            ...
            switch (rule.operator) {
              case "<=":
                if (order_details.amount <= value) {
                  rule_eval_obj.no_of_rule_pass =
                    rule_eval_obj.no_of_rule_pass + 1;
                } else {
                  rule_eval_obj.no_of_rule_fail =
                    rule_eval_obj.no_of_rule_fail + 1;
                }
                break;
              case ">=":
                if (order_details.amount >= value) {
          ...
          if (key.trim() == "merchant_country") {
            ...
            if (rule.operator == "=") {
              ...
              if (
                value.toUpperCase() ==
                country_details.country_code.toUpperCase()
              ) {
                rule_eval_obj.no_of_rule_pass =
                  rule_eval_obj.no_of_rule_pass + 1;
              } else {
          ...
          if (key.trim() == "card_country") {
            ...
            if (rule.operator == "=") {
              if (
                req.card_details.country_code3.toUpperCase() ==
                value.toUpperCase()
              ) {
                rule_eval_obj.no_of_rule_pass =
                  rule_eval_obj.no_of_rule_pass + 1;
          ...
          if (key.trim() == "card_type") {
            if (rule.operator == "=") {
              if (
                req.card_details.card_type.toUpperCase() == value.toUpperCase()
              ) {
                rule_eval_obj.no_of_rule_pass =
                  rule_eval_obj.no_of_rule_pass + 1;
              } else {
          ...
          if (key.trim() == "card_scheme") {
            if (rule.operator == "=") {
              if (
                req.card_details.card_brand.toUpperCase() == value.toUpperCase()
              ) {
                rule_eval_obj.no_of_rule_pass =
                  rule_eval_obj.no_of_rule_pass + 1;
              } else {
          ...
          if (key.trim() == "transaction_type") {
            ...
            if (
              country_details.country_code == req.card_details.country_code3
            ) {
              type = "DOMESTIC";
            } else {
              type = "INTERNATIONAL";
          ...
          if (key.trim() == "3ds_version") {
            let card_proxy = await getCardProxyByCardIdOrCardNo(
              enc_dec.cjs_decrypt(card_id),
              card
            );

            // console.log("card_proxy", card_proxy);

            let versionDetails = await helpers.fetch3dsVersion({
      ...
      first_selected_mid = await decideRuleRouting(
        rule_evaulation,
        routing_order,
        order_details.order_id,
        payment_mode,
    ...
    if (first_selected_mid) {
      const _terminal_details = await merchantOrderModel.selectDynamicONE(
        "*",
        { id: first_selected_mid },
        "mid"
      );
      ...
      const base_url =
        payment_mode == "test"
          ? psp_credentials.test_url
          : psp_credentials?.base_url;
      if (
        order_urls.success != "" &&
        order_urls.success != null &&
        order_urls.success != "undefined"
      ) {
        redirect_url.success = order_urls.success;
      } else if (
        _terminal_details?.success_url != "" &&
        _terminal_details?.success_url != null &&
        _terminal_details?.success_url != "undefined"
      ) {
      ...
      if (
        order_urls.failure != "" &&
        order_urls.failure != null &&
        order_urls.failure != "undefined"
      ) {
        redirect_url.failure = order_urls.failure;
      } else if (
        _terminal_details?.failure_url != "" &&
        _terminal_details?.failure_url != null &&
        _terminal_details?.failure_url != "undefined"
      ) {
      ...
      if (
        order_urls.cancel != "" &&
        order_urls.cancel != null &&
        order_urls.cancel != "undefined"
      ) {
        redirect_url.cancel = order_urls.cancel;
      } else if (
        _terminal_details?.cancel_url != "" &&
        _terminal_details?.cancel_url != null &&
        _terminal_details?.cancel_url != "undefined"
      ) {
      ...
      let updateorder = {
        remark:
          order_details.remark == ""
            ...
            : order_details.remark,
        action:
          order_details?.origin == "REMOTE"
            ? order_details?.action
            : _terminal_details?.mode,
      ...
      if (req.body.type == "routing") {
        ...
        let last_transaction_r = await helpers.lastTwoCardUsed({
          order_id: order_id,
          mode: payment_mode,
        });
        let isOrderRetry = await merchantOrderModel.selectDynamicONE(
          "*",
          { order_id: order_id, mode: payment_mode },
          "order_life_cycle"
        );
        ...
        if (isOrderRetry.retry == 0 && !retry_txn) {
          ...
          return res
            .status(StatusCode.ok)
            .send(
              ServerResponse.errorMsgWithData("Transaction failed.", res_obj_f)
            );
        }
      }
      if (retry_txn) {
        ...
        return {
          status: "success",
          message: "payment psp found successfully",
          data: psp_details,
        };
      } else {
        let res_obj = {
...
async function findOutWhereToRoute(orderLifeCycle) {
  let transactionLifeCycle = await merchantOrderModel.selectAllDynamic(
    "id,status",
    { order_id: orderLifeCycle.order_id, status: 1 },
  ...
  switch (transactionLifeCycle.length) {
    ...
    default:
      console.log("retry2");
      let last_transaction = await helpers.lastTwoCardUsed({
        order_id: orderLifeCycle.order_id,
        mode: orderLifeCycle.mode,
      });
      update_retry(last_transaction);
      ...
      if (retry_no > transactionLifeCycle.length) {
        ...
        if (
          retry_no + 1 + cascade_no * retry_no - transactionLifeCycle.length >
          0
        ) {
          ...
          let no_of_cascade_happened =
            transactionLifeCycle.length - (retry_no + 1);
          if (no_of_cascade_happened > 0) {
            return mid_list[
              transactionLifeCycle.length - (retry_no * cascade_no - 1)
            ];
          } else {
...
async function decideRuleRouting(
  rule_evaulation,
  routing_order,
  order_id,
  payment_mode,
  ...
  switch (rule_evaulation.length) {
    case 0:
      if (routing_order.length > 0) {
        let firstSelectedMid = await routingOrderBasedMidList(
          routing_order,
          order_id,
          payment_mode,
          card_details,
    ...
    case 1:
      if (rule_evaulation[0].no_of_rule_fail == 0) {
        let firstSelectedMid = await midListFromRule(
          rule_evaulation[0],
          routing_order,
          order_id,
          payment_mode,
      ...
      break;
    default:
      let firstSelectedMid = await minFailedRuleMid(
        rule_evaulation,
        routing_order,
        order_id,
        payment_mode,
...
async function terminalRouting(req, order_id, payment_mode) {
  try {
    ...
    const midquery = `SELECT md.* , mc.code , mc.currency  FROM pg_mid md INNER JOIN pg_master_currency mc ON mc.id = md.currency_id WHERE md.submerchant_id = '${parseInt(
      order_details.merchant_id
    )}'   AND  md.status = 0 AND md.deleted = 0 AND md.env ='${payment_mode}' AND FIND_IN_SET('${card_type}', md.payment_schemes) AND FIND_IN_SET('${card_dc}',md.payment_methods) AND  mc.code = '${
      order_details?.currency
    }';`;
    console.log(midquery);
    const getmid = await merchantOrderModel.order_query(midquery);
    ...
    const branding_query = `SELECT others,is_visible FROM pg_merchant_payment_methods WHERE methods='card_payment' AND sub_merchant_id='${parseInt(
      order_details.merchant_id
    )}'`;
    const branding_controls = await merchantOrderModel.order_query(
      branding_query
    );
    ...
    if (getmid.length > 0) {
      if (getmid.length === 1) {
        if (payment_mode == "live") {
          ...
          //   return {
          //     status: "failed",
          //     message: "Domestic cards is not supported by this merchant!",
          //   };
          // }
          // if (
          //   is_domestic_or_international == "International" &&
          ...
          //   return {
          //     status: "failed",
          //     message: "International cards is not supported by this merchant!",
          //   };
          // }
        }
        const getpsp = await PspModel.selectOne("*", {
        ...
        if (!getpsp.credentials_key) {
          return {
            status: "failed",
            message: "No Credentials Found in Selected PSP!!",
          };
        }
        const psp_name = getpsp.credentials_key.toLowerCase();
        ...
        if (
          ...
          getmid[0]?.success_url != "" &&
          getmid[0]?.success_url != null &&
          getmid[0]?.success_url != "undefined"
        ) {
        ...
        if (
          ...
          getmid[0]?.failure_url != "" &&
          getmid[0]?.failure_url != null &&
          getmid[0]?.failure_url != "undefined"
        ) {
        ...
        if (
          ...
          getmid[0]?.cancel_url != "" &&
          getmid[0]?.cancel_url != null &&
          getmid[0]?.cancel_url != "undefined"
        ) {
        ...
        return {
          status: "success",
          message: "PSP found successfully.",
          data: psp_details,
        };
      } else {
        const mid_name = [];
        ...
        for (let i = 0; i < getmid.length; i++) {
          ...
          const query = `SELECT  SUM(CASE WHEN status = 'AUTHORISED' THEN 1 ELSE 0 END) AS success_count, SUM(CASE WHEN status = 'FAILED' THEN 1 ELSE 0 END) AS failure_count FROM ${
            config.table_prefix + table_name
          } WHERE psp = '${getpspName.credentials_key.toUpperCase()}' AND DATE(created_at) BETWEEN  '${last_week_day}' AND '${today}'`;
        ...
        if (
          ...
          maxObj?.success_url != "" &&
          maxObj?.success_url != null &&
          maxObj?.success_url != "undefined"
        ) {
        ...
        if (
          ...
          maxObj?.failure_url != "" &&
          maxObj?.failure_url != null &&
          maxObj?.failure_url != "undefined"
        ) {
        ...
        if (
          ...
          maxObj?.cancel_url != "" &&
          maxObj?.cancel_url != null &&
          maxObj?.cancel_url != "undefined"
        ) {
...
async function mpgsSessionAndAuth(req, order_id, mode, psp_name) {
  console.log(`this is card details in s2s`);
  ...
  let order_data = {
    ...
    card_id:req.card_details.card_id,
  ...
  let mpgs_req = {
    ...
    order_id: order_id,
    card_no: req.body.paymentMethod.paymentCard.number,
    expiry_date: req.body.paymentMethod.paymentCard.expiryDate
      .split("/")
      .reverse()
      .join("-"),
  ...
  try {
    let url =
      mode == "live"
        ? credentials[getpsp.credentials_key].base_url
        : credentials[getpsp.credentials_key].test_url;
    const config1 = {
      method: "post",
    ...
    let auth_res = {
      threeds_url:
        ...
        "/mpgs-process-threeds?session_id=" +
        sessionId +
        "&order_id=" +
        order_id +
        "&transaction_id=" +
...
async function routingOrderBasedMidList(
  routing_order,
  order_id,
  mode,
  card_details,
  ...
  let add_res = await RoutingModel.add(
    order_life_cycle_data,
    "order_life_cycle"
  );
...
async function saveCard(req, order_id, mode) {
  try {
    ...
    let card_proxy = enc_dec.encrypt_card(
      req.body.paymentMethod.paymentCard.number
    );
    // check if card exits in database
    let isCardExits = await MerchantRegistrationModel.selectOneDyn('id',{card_number:card_number},'customers_cards');
    if(isCardExits){
    ...
    console.log(`the result is here of save card`);
    console.log(result);
...
async function mtnPay(data,mode,order_id){
   let payment_id;
  let psp = data.paymentMethod.wallet_details.walletType;
  let order_table = mode == "live" ? "orders" : "test_orders";
  let txn_table = mode == "live" ? "order_txn" : "test_order_txn";
  let txn_response_dump =
    mode == "live" ? "txn_response_dump" : "test_txn_response_dump";
  ...
  // fetch psp id
  let psp_details = await fetchPSPDetails(psp);
  if (!psp_details) {
    return { status: 'failed', message: "No Psp Available", data: [] }
  ...
  const mid_details = await fetchMidDetails( psp_details.id,order_details.merchant_id,mode);
  if (!mid_details) {
     return { status: 'failed', message: "No Terminal Available", data: [] }
  }
  ...
  let order_data = {
    ...
    cardType: 'NA',
    scheme: '',
    pan: `${data.paymentMethod.wallet_details.mobileCode}${data.paymentMethod.wallet_details.msisdn}`,
    cardholderName: 'NA',
    expiry: 'NA',
  ...
  try {
    token = await mtnService.token(mid_details.primary_key,mid_details.MID,mid_details.password,mode);

    if(!token){
        return { status: 'failed', message: "Unable to authenticate", data: [] }
    }
    payment_id = await helpers.make_sequential_no(
      mode == "test" ? "TST_TXN" : "TXN"
    );
    ...
    let x_ref_id = await mtnService.pay(order_details,data,mid_details,mode,token);
    if(!x_ref_id){
       return { status: 'failed', message: "Unable to initiate payment", data: [] }
    }
    // Call updateDynamic to store sessionId in the database
    await merchantOrderModel.updateDynamic(
    ...
    if (!insert_to_txn_table) {
       return { status: 'failed', message:"Transaction insertion failed", data: [] }
    }

    let authRes = {
      threeds_url:"NA",
      mpgs_url: "NA", // from config file
      session_id: x_ref_id, // from meps api
      order_id: order_id, // paydart order id
      transaction_id: payment_id.toString(),
      mid: mid_details.MID, // From routing,
      billing_address: data?.billing_details?.address_line1,
      city:  data?.billing_details?.city,
      billing_country:  data?.billing_details?.country,
    };
    return{
      status: "Pending",
    ...
    return {status:'failed',message:res_obj.message,data:[]} } 
}
async function orangePay(data,mode,order_id){
   let payment_id;
   let psp = data.paymentMethod.wallet_details.walletType;
   let order_table = mode == "live" ? "orders" : "test_orders";
   let txn_table = mode == "live" ? "order_txn" : "test_order_txn";
   ...
   let generate_request_id_table =
    ...
    // fetch psp id
     ...
     const mid_details = await fetchMidDetails(
       psp_details.id,
       order_details.merchant_id,
       mode
     );
   ...
   try {
     ...
     const order_date_update = await merchantOrderModel.updateDynamic(order_data,{order_id: order_id},order_table);
     const final_response = await orangeService.pay(order_details,data,mid_details,mode);
     if(!final_response.data?.resultset?.TXNID){
       return {status:'failed',message:final_response?.data?.exec_msg,data:[]}
     }
      // Call updateDynamic to store sessionId in the database
    await merchantOrderModel.updateDynamic(
      { payment_id: payment_id,pan:`${data.paymentMethod.wallet_details.msisdn}` },
      { order_id: order_id },
      order_table
    );
     payment_id = await helpers.make_sequential_no(
    ...
    if (!insert_to_txn_table) {
     ...
     let authRes = {
      threeds_url:"NA",
      mpgs_url: "NA", // from config file
      session_id: final_response.data?.resultset?.TXNID, // from meps api
      order_id: order_id, // paydart order id
      transaction_id: payment_id.toString(),
      mid: mid_details.MID, // From routing,
      billing_address: data?.billing_details?.address_line1,
    ...
    const res_obj = {
      ...
      new_res: {
        ...
        payment_method: order_details?.payment_mode || "",
        m_payment_token: order_details?.m_payment_token || "",
        mobile_no:`${data.paymentMethod.wallet_details.mobileCode}${data.paymentMethod.wallet_details.msisdn}`,
        payment_method_data: {
          scheme: order_details?.scheme || "",
          card_country: order_details?.card_country || "",
          card_type: "Mobile Wallet",
...
async function alpay(data,mode,order_id){
   let payment_id;
   let psp = data.paymentMethod.wallet_details.walletType;
   let order_table = mode == "live" ? "orders" : "test_orders";
   let txn_table = mode == "live" ? "order_txn" : "test_order_txn";
   ...
   let generate_request_id_table =
    ...
    // fetch psp id
     let psp_details = await fetchPSPDetails('alpay');
     if (!psp_details) {
       return { status: "failed", message: "No Psp Available", data: [] };
     }
   ...
   try {
     ...
     const order_date_update = await merchantOrderModel.updateDynamic(order_data,{order_id: order_id},order_table);
    //  fetch token
     let token = await alpayService.token(mid_details.MID,mid_details.password,mode);
    //  fetch account holder name
     let verficiationDetails = await alpayService.verify(data,token,mode);
     let debitNarration = ''
      if (
        ...
        debitNarration = `PAY TO ${merchantName.company_name}`;
      }else{
        debitNarration = mid_details.statementDescriptor;
      }
    //  make a payment
     const final_response = await alpayService.pay(verficiationDetails,order_details,data,mode,token,debitNarration);
     if(!final_response){
       return {status:'failed',message:final_response?.data?.message,data:[]}
     }
    ...
    const insert_to_txn_table = await insertFunction({
      ...
      currency: order_details.currency,
      created_at: moment().format("YYYY-MM-DD HH:mm:ss"),
      payment_id: final_response.data?.data?.transactionId,
    });
    if (!insert_to_txn_table) {
       return{
      status: "failed",
     ...
     let authRes = {
      threeds_url:"NA",
      mpgs_url: "NA", // from config file
      session_id: final_response.data?.data?.transactionId, // from meps api
      order_id: order_id, // paydart order id
      transaction_id: payment_id.toString(),
      mid: mid_details.MID, // From routing,
      billing_address: data?.billing_details?.address_line1,
...
function mapToWalletPayRequest(req,mode,order_id) {
  // Extracting from new request body and mapping fields
  const walletPayRequest = {
    country_code: req.body.paymentMethod.wallet_details.mobileCode,
    mobile_no: req.body.paymentMethod.wallet_details.msisdn,
    mode: mode,           // no mapping provided, default empty
    order_id: order_id,       // no mapping provided, default empty
    psp: req.body.paymentMethod.wallet_details.walletType,
    mno: req.body.paymentMethod.wallet_details.walletType,
    account_name: ""    // no mapping provided, default empty
  };

  return walletPayRequest;
}
async function fetchPSPDetails(psp){
...
async function fetchMidDetails(psp_id,merchant_id,mode){
 let mid_details =   await merchantOrderModel.selectOne(
...
async function mtnSandboxPay(data,mode,order_id){
   let payment_id;
  let psp = data.paymentMethod.wallet_details.walletType;
  let order_table = mode == "live" ? "orders" : "test_orders";
  let txn_table = mode == "live" ? "order_txn" : "test_order_txn";
  ...
  try {
    token = await mtnSandboxService.token(mid_details.primary_key,mid_details.MID,mid_details.password,mode);
    ...
    // call to make payment 
    let x_ref_id = await mtnSandboxService.pay(order_details,data,mid_details,mode,token);
    if(!x_ref_id){
       return { status: 'failed', message: "Unable to initiate payment", data: [] }
    }
    // Call updateDynamic to store sessionId in the database
...
async function orangeSandboxPay(data,mode,order_id){
   let payment_id;
   let psp = data.paymentMethod.wallet_details.walletType;
   let order_table = mode == "live" ? "orders" : "test_orders";
   let txn_table = mode == "live" ? "order_txn" : "test_order_txn";
   ...
   try {
     ...
     const order_date_update = await merchantOrderModel.updateDynamic(order_data,{order_id: order_id},order_table);
     console.log(`beofre orange sends`);
     const final_response = await orangeSandboxService.pay(order_details,data,mid_details,mode);
     if(!final_response.data?.resultset?.TXNID){
       return {status:'failed',message:final_response?.data?.exec_msg,data:[]}
     }
      // Call updateDynamic to store sessionId in the database
