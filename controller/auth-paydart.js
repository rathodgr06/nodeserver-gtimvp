...
var Auth = {
  login: async (req, res) => {
    ...
    let enc_username = encrypt_decrypt("encrypt", username);
    let enc_password = encrypt_decrypt("encrypt", password);
    let foundUser = await AdminModel.selectOne("*", {
    ...
    if (foundUser) {
      ...
      let passwordLogs = await AdminModel.selectPasswordLogs("*", {
        user_type: "admin",
        user_id: foundUser.id,
      });

      module_and_user.user = foundUser.id;

      if (passwordLogs) {
        lastPasswordDate = passwordLogs.added_date;
      }

      let lpd = await date_formatter.get_default_date(lastPasswordDate);
      let dateResult = await date_formatter.get_diff_date(lpd);

      let user = foundUser;
      if (user.is_blocked == 1) {
        module_and_user.title = "Invalid login attempt. User is blocked.";
        res.status(statusCode.ok).send(response.errormsg("User is blocked."));
      } else if (user.status == 1) {
        module_and_user.title = "Invalid login attempt. User is not active.";
        res
          .status(statusCode.ok)
          .send(response.errormsg("User is not active."));
      } else if (dateResult >= 90) {
        payloadData = {
          id: user.id,
          type: "admin",
        ...
        frontend_link = admin_link + "/reset?auth=" + aToken;
        ...
        module_and_user.title =
          "Invalid login attempt. Your password is expired. Please reset password.";
        res
          .status(statusCode.ok)
          .send(
        ...
        /*** Two FA for admin end *********************************************/
        module_and_user.title = `Valid login attempt. Please verify 2FA`;
        res
          .status(statusCode.ok)
          .send(
        ...
        admin_activity_logger
          .add(module_and_user, module_and_user.title, headers)
          .then((result) => {})
          .catch((error) => {
            winston.error(error);
          });
        ...
        /*  const aToken = accessToken(payload);
                ...
                res.status(statusCode.ok).send(response.loginSuccess({ accessToken: aToken, name: payload.name, 'language': language, 'theme': user.theme, 'user_type': 'admin', user_id: user_id })); */
      }
    } else {
      let adm_email = await helpers.get_admin_email_by_username(enc_username);
    ...
    AdminModel.select2falogin({ token: token })
      .then(async (result) => {
        ...
        if (verification_result) {
          // if (req.bodyString("pin") == "123456" /*verification_result*/) {
          ...
          let language = await nodeCache.getActiveLanguageById(user.language);
  ...
  forget_password: async (req, res) => {
    ...
    if (foundUser) {
      ...
      const aToken = customaccessToken(payload, 900);
