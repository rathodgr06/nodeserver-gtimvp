...
const { payment_id } = require('../order');
...
const confirm_payment = async (req, res) => {
    ...
    try {
        ...
        // generate token
        let url1 = mode == "live" ? credentials[psp_details.credentials_key].base_url : credentials[psp_details.credentials_key].test_url;
           const config1 = {
             method: "post",
        ...
        let txnFailedLog = {
            ...
            status_code: final_response.data.data.status == "SUCCESSFUL" ? "00" : "01",
            description: "",
            activity: `Transaction ${final_response.data.data.status === 'SUCCESSFUL' ? 'SUCCESS' : 'FAILED'} with ALPAY`,
        ...
        if (final_response.data.data.status == 'SUCCESSFUL' &&  mode==process.env.CHARGES_MODE) {
            const transaction_and_feature_data = {
                ...
                txn_status: (final_response.data.data.status == 'SUCCESSFUL') ? "AUTHORISED" : "FAILED",
        ...
        const responseStatus = final_response?.data?.data.status == 'SUCCESSFUL' ? statusCode.ok : statusCode.badRequest;
