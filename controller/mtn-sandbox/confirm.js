...
const confirm_payment = async (req, res) => {
    ...
    try {
        ...
        // generate token
        let url_token = mode == "live" ? credentials[psp_details.credentials_key].base_url : credentials[psp_details.credentials_key].test_url;
        const config1 = {
            method: "post",
            url: url_token + `momo-sandbox/token/`,
            headers: {
        ...
        let config = {
            method: 'get',
            url: `${url}momo-sandbox/v1_0/requesttopay/${order_txn_details.payment_id}`,
            headers: {
                'Content-Type': 'application/json',
        ...
        let txnFailedLog = {
            ...
            description: "",
            activity: `Transaction ${final_response.data.status === 'SUCCESSFUL' ? 'SUCCESS' : 'FAILED'} with MPGS`,
        ...
        if (final_response.data.status == 'SUCCESSFUL' /*&& mode == "live"*/) {
            const transaction_and_feature_data = {
                ...
                txn_status: (final_response.data.status == 'SUCCESSFUL') ? "AUTHORISED" : "FAILED",
        ...
        const responseStatus = final_response?.data?.status == 'SUCCESSFUL' ? statusCode.ok : statusCode.badRequest;
