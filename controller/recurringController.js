...
let RecurringController = {
  live: async () => {
    try {
      console.log(`Running per minute`);
      ...
      if (current_payments.length > 0) {
        let promise_arr = current_payments.map(async (val) => {
          ...
          if (order_details && order_details.length > 0) {
            ...
            switch (psp.toUpperCase()) {
              case "TELR":
                await telr_pay(response, "live");
                break;
              case "NI":
                await ni_pay(response, "live");
                break;
              case "PAYTABS":
                await paytabs_pay(response, "live");
                break;
            }
          } else {
          }
        ...
        console.log("All recurring payment captured");
      } else {
        console.log("No recurring payment");
      }
    } catch (error) {
      // winston.error(error);
      console.log(error);
  ...
  RecurringOpenApi: async (req, res) => {
    try {
      const {
        action,
        transaction_id,
  ...
  RecurringOpenApiTest: async (req, res) => {
    try {
      const {
        action,
        transaction_id,
      ...
      responseObj.payment_ref_id = transaction_id;
      responseObj.token = paid_data?.response;
      responseObj.current_entry = val;
  ...
  test: async () => {
    try {
      let current_payments = await helpers.get_current_data_test();
      ...
      if (current_payments.length > 0) {
        let promise_arr = current_payments.map(async (val) => {
          ...
          // console.log(paid_data[0]?.order_id);return;

          let order_details = await helpers.get_data_list1("*", "test_orders", {
            order_id: paid_data[0]?.order_id,
          });

          response.payment_ref_id = paid_data[0]?.payment_id;
          (response.token = paid_data[0]?.response),
            (response.current_entry = val);
          response.order_details = order_details[0];
          //console.log(order_details);return;

          if (
            order_details &&
            order_details.length > 0 &&
            order_details.saved_card_for_recurring != ""
          ) {
            let psp = order_details[0]?.psp;
            // console.log(psp);
            switch (psp.toUpperCase()) {
              case "TELR":
                console.log(psp);
                await telr_pay(response, "test");
                break;
              case "NI":
                await ni_pay(response, "test");
                break;
              case "PAYTABS":
                await paytabs_pay(response, "test");
                break;
              case "MPGS":
                await mpgs_pay(response, "test");
                break;
              case "FISERV":
                await fiserv_pay(response, "test");
                break;
            }
          } else {
          }
...
async function telr_pay(req, mode) {
  ...
  payment_id = payment_id + randomNumber;
    ...
    merchantOrderModel
      ...
      .then(async (result) => {
        ...
        let update_current_entry = {
          ...
          response: JSON.stringify(sale_api_res),
          order_id: res_order_data.order_id,
        };
        ...
        if (sale_api_res.status === "A") {
          ...
          let order_life_cycle = {
            status: 0,
            mode: mode,
            activity: "Subscription",
            txn: payment_id,
            order_id: res_order_data.order_id,
            terminal: _terminalids.terminal_id,
          ...
          const invoice_payment = await invModel.selectDynamic(
            "id",
            { order_id: res_order_data.order_id },
            "inv_invoice_master"
          );
...
async function old_telr_pay(req, mode) {
  let updated_at = moment().format("YYYY-MM-DD HH:mm:ss");
  let payment_id = await helpers.make_sequential_no(
    mode == "test" ? "TST_TXN" : "TXN"
  );
...
async function ni_pay(req, mode) {
  try {
    ...
    // console.log("req.order_details.order_id");
    //console.log(req.order_details.order_id);
    const previous_order_id = req.order_details.order_id;
    const previous_order_data = await merchantOrderModel.selectOne(
      "*",
      {
    ...
    // console.log("previous_order_data")
    //console.log(previous_order_data)
    ...
    let payload = {
      action: "SALE",
      amount: {
        value: parseFloat(res_order_data.amount) * 100,
    ...
    console.log("previous_order_data.saved_card_for_recurring");
    console.log(previous_order_data.saved_card_for_recurring);
    ...
    let paymentRecurringNi = await NI.captureRecurring(
      ...
      orderReferenceNo,
      paymentNo,
      JSON.parse(previous_order_data.saved_card_for_recurring)
    );
    let payment_id = await helpers.make_sequential_no(
      mode == "test" ? "TST_TXN" : "TXN"
    );
    ...
    let order_life_cycle = {
      ...
      res: JSON.stringify(previous_order_data.saved_card_for_recurring),
      created_at: moment().format("YYYY-MM-DD HH:mm:ss"),
      psp: "NI",
    };

    //console.log(order_life_cycle);
    await orderTransactionModel.add(order_life_cycle, "order_life_cycle_logs");
...
async function paytabs_pay(req, mode) {
  ...
  let data = JSON.stringify({
    ...
    cart_currency: res_order_data.currency,
    cart_amount: res_order_data.amount,
  });
  ...
  let updateRecurring = {
    is_paid: paytabs_res.payment_result.response_status == "A" ? 1 : 0,
    payment_id: paytabs_res.tran_ref,
    response: req.token,
    is_failed: paytabs_res.payment_result.response_status == "A" ? 0 : 1,
    order_id: res_order_data.order_id,
  };
  let p_request_id = await helpers.make_sequential_no(
    mode == "test" ? "TST_REQ" : "REQ"
...
async function mpgs_pay(req, mode) {
  let order_table = "orders";
  let txn_table = "order_txn";
  if (mode == "test") {
    order_table = "test_orders";
  ...
  let payment_mode = res_order_data.payment_mode;
  let subscription_id = req?.current_entry?.subscription_id;
  console.log(`subscription id is here`);
  ...
  try {
    ...
    merchantOrderModel
      ...
      .then(async (result) => {
        ...
        sale_api_res = await mpgsTestRecurring(sale_payload, _terminalcred);
        ...
        let update_current_entry = {
          payment_id: sale_api_res?.sourceOfFunds?.token
            ? sale_api_res?.sourceOfFunds?.token
            : "",
          is_paid: sale_api_res.result === "SUCCESS" ? 1 : 0,
          is_failed: sale_api_res.result === "SUCCESS" ? 0 : 1,
          response: JSON.stringify(sale_api_res),
          order_id: res_order_data.order_id,
        };
        ...
        let payment_status =
          sale_api_res.result === "SUCCESS" ? "AUTHORISED" : "FAILED";
        await subscription_card_expired_model.lastSubscriptionPayment(
          subscription_id,
          payment_status
        );
        /* Update the payment status for various payment channel end */
        if (sale_api_res.result === "SUCCESS") {
          let status = "AUTHORISED";
          //adding entry to pg_order_txn table
          ...
          let order_txn = {
            status: status,
            txn: payment_id,
            type: sale_api_res.order.type,
            payment_id: sale_api_res.order.reference,
            order_id: res_order_data.order_id,
            amount: req.current_entry.amount,
            currency: res_order_data.currency,
            created_at: updated_at,
          ...
          let new_res = {
            ...
            psp_txn_id: sale_api_res?.tranref,
            transaction_id: payment_id,
            status: sale_api_res.result === "SUCCESS" ? "SUCCESS" : "FAILED",
            status_code: "00",
            remark: sale_api_res?.message,
            paydart_category: response_category.category,
            currency: res_order_data.currency,
...
async function fiserv_pay(req, mode) {
  let order_table = "orders";
  let txn_table = "order_txn";
  if (mode == "test") {
    order_table = "test_orders";
  ...
  // create new order and fetch order details
  let order_id = await createOrder(req.order_details.order_id, order_table);

  const res_order_data = await merchantOrderModel.selectOne("*",{order_id: order_id,},order_table);
  ...
  try {
    ...
    //fetch transaction which are due
    let checkTransaction = await orderTransactionModel.getDueDateTransaction(
      order_id,
      subscription_id
    );
    ...
    let result = await merchantOrderModel.updateDynamic(order_data, { order_id: order_id, }, table_name);

    const _terminalids = await merchantOrderModel.selectOne("terminal_id", { order_id: order_id, }, table_name);
    const _getmid = await merchantOrderModel.selectOne("MID,password,psp_id", { terminal_id: _terminalids.terminal_id, }, "mid");

    // send request to fiserv for recurring 
    let rawRequest = {
      "requestType": "PaymentTokenSaleTransaction",
      "transactionAmount": {
        "total": res_order_data.amount,
        "currency": res_order_data.currency
      },
      "paymentMethod": {
        "paymentToken": {
          "value": res_order_data.payment_token_id
        }
      },
      "storedCredentials": {
        "sequence": "SUBSEQUENT",
    ...
    let url = mode == "test" ? credientials['fiserv']['test_url'] + 'payments' : credientials['fiserv']['base_url'] + 'payments';
    console.log(_getmid);
    let fiservRequest = RequestMaker('POST',url, rawRequest, _getmid.MID, _getmid.password);

    let recurringResponse = await axios.post(fiservRequest.url, fiservRequest.body, {
    ...
    console.log(recurringResponse);
     let order_req = {
        ...
        order_id: res_order_data.order_id,
        request_id: '',
        request: JSON.stringify(rawRequest),
      };
      await helpers.common_add(
        order_req,
        mode == "test" ? "test_generate_request_id" : "generate_request_id"
      ...
      let update_current_entry = {
        payment_id:recurringResponse.data.ipgTransactionId,
        is_paid: recurringResponse.data.transactionStatus == "APPROVED" ? 1 : 0,
        is_failed: recurringResponse.data.transactionStatus == "APPROVED" ? 0 : 1,
        response: JSON.stringify(recurringResponse.data),
        order_id: res_order_data.order_id,
      };
      ...
      let payment_status =
        recurringResponse.data.transactionStatus == "APPROVED"? "AUTHORISED" : "FAILED";
      await subscription_card_expired_model.lastSubscriptionPayment(
        subscription_id,
        payment_status
      );
      // Update the payment status for various payment channel end 
      if (recurringResponse.data.transactionStatus === "APPROVED") {
        let status = "AUTHORISED";
        //adding entry to pg_order_txn table
        ...
        let order_txn = {
          ...
          currency: res_order_data.currency,
          created_at: updated_at,
          order_reference_id: recurringResponse.data.ipgTransactionId,
          capture_no: "",
          subscription_id: req.current_entry.subscription_id,
        };
        ...
        let order_life_cycle = {
          ...
          order_id: res_order_data.order_id,
          terminal: _terminalids.terminal_id,
          res: JSON.stringify(recurringResponse.data),
          created_at: updated_at,
          psp: "FISERV",
        };
        ...
        let response_dump = {
          ...
          type: res_order_data.action,
          status: status,
          dump: JSON.stringify(recurringResponse.data),
        };
        ...
        let new_res = {
          ...
          p_order_id: res_order_data.order_id,
          p_request_id: '',
          psp_ref_id: recurringResponse.data.apiTraceId,
          psp_txn_id: recurringResponse.data.ipgTransactionId,
          transaction_id: payment_id,
          status: recurringResponse.data.transactionStatus === "APPROVE" ? "SUCCESS" : "FAILED",
          status_code: "00",
          remark: "",
          paydart_category: response_category.category,
          currency: res_order_data.currency,
