...
async function getMerchantSellRate(
  ...
  let enc_payment_scheme_id = await helpers.getPaymentSchemeEnc(is_mobile_wallet?"MOBILE WALLET":scheme);
    if(is_mobile_wallet){
       condition= {
      "LOWER(pricing_plan.payment_methods)": `PAY BY MOBILE MONEY`,
  ...
  // }
    ...
    try {
      ...
      .select(
        `
        pricing_plan.id as pricing_plan_id,
        pricing_plan.sale_rate_fix,
        pricing_plan.sale_rate_per,
        pricing_plan.tax,
        pricing_plan.min_amount,
        pricing_plan.max_amount
        `
      )
      .from(`${mid_table} mid`)
      .join(
  ...
  return {
    pricing_plan_id:pricing_plan_id||0,
...
async function getMerchantDefaultSellRate( details, is_domestic_international ) {
  console.log("🚀 ~ getMerchantDefaultSellRate ~ details:1234", details)
  const { merchant_id, currency, scheme, payment_method, terminal_id, psp_id, country_id,is_mobile_wallet } = details;
