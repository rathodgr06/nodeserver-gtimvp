...
var mailSender = {
  welcomeMailOld: async (mail, subject, url) => {
    ...
    let template = await helpers.getSingleMailTemplate("test");
    let template_html = template.template;
    let template_subject = template.subject;
  ...
  welcomeMail: async (mail, subject, url) => {
    try {
      ...
      let image_path = server_addr + "/static/images/";
      let logo = smtp_details.company_logo
        ? image_path + smtp_details.company_logo
        : "";
      ...
      let finalSubject = helpers.applyTemplateVars(
        template_subject,
        templateVars
      );

      let finalHtml = helpers.applyTemplateVars(template_html, templateVars);

      let info = await transporter.sendMail({
        from: smtp_details.smtp_from,
        to: mail,
        subject: finalSubject || title + " - " + subject,
        html: finalHtml,
      });
    } catch (error) {
      console.error(error);
