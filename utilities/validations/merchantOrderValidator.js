...
const MerchantOrderValidator = {
                ...
                case 'CONT':
                    try {
                        ...
                        let transactionSchema = Joi.object().keys({
                            class: Joi.string().valid('CONT').error(() => {
                                return new Error("Class not valid/not supplied");
                            }),
                            action: Joi.string().valid('SALE').error(() => {
                                return new Error("Order action not valid/not supplied");
                            }),
                            transaction_id: Joi.string().required().error(() => {
                        ...
                        if (result1.error) {
                            ...
                            if (mode == 'test') {
                                record_exits = await checkifrecordexist({ txn: req.body.data.transaction_id }, 'test_order_txn')
                            } else {
                                record_exits = await checkifrecordexist({ txn: req.body.data.transaction_id }, 'order_txn')
                            }
                            if (record_exits) {
                                next();
                            } else {
    ...
    test_create: async (req, res, next) => {
        try {
            ...
            res.status(StatusCode.badRequest).send(
                ServerResponse.validationResponse(err.message)
            );
        }
    },
    ...
    open_get: async (req, res, next) => {
        ...
        try {
            ...
            if (result.error) {
                ...
                if (req.bodyString("order_id")) {
                    let order_id = req.bodyString("order_id");
                    let table_name = req?.credentials?.type == "test" ? "test_orders" : "orders";
                    ...
                    if (!order_exits) {
                        let payload = {
                            psp_name: "paydart",
                            psp_response_details:
                                "Order id not valid/not supplied",
                        };
                        let common_err = await helpers.get_common_response(
                            payload
                        );
    ...
    telr_pay: async (req, res, next) => {
        const schema = Joi.object().keys({
            ...
            env: Joi.string().required().error(() => {
             ...
             width: Joi.string().required().error(() => {
                 return new Error("Width required");
             }), height: Joi.string().required().error(() => {
                 return new Error("Height required");
             }),
            agent: Joi.string().required().error(() => {
        ...
        if (result.error) {
            ...
            if (req.bodyString("order_id")) {
                ...
                if (!order_exits) {
                    res.status(StatusCode.badRequest).send(
                        ServerResponse.validationResponse("Invalid order id")
                    );
    ...
    pay: async (req, res, next) => {
        ...
        if (result.error) {
            ...
            if (req.bodyString("order_id")) {
                ...
                res.status(StatusCode.badRequest).send(
                    ServerResponse.validationResponse("Valid order id required")
                );
            }
        }
    },
            ...
            apple_token: Joi.object({
                ...
                header: Joi.object({
                    publicKeyHash: Joi.string()
                        ...
                        .error(() => {
                            return new Error(
                                "Header Public Key Hash is required"
                            );
                        }),
                    ephemeralPublicKey: Joi.string()
                        ...
                        .error(() => {
                            return new Error(
                                "Header Ephemeral Public Key is required"
                            );
                        }),
                    transactionId: Joi.string()
                        ...
                        .error(() => {
                            return new Error(
                                "Header Transaction ID is required"
                            );
                        }),
                })
                    .required()
        ...
        try {
            ...
            if (result.error) {
                ...
                if (req.bodyString("order_id")) {
                    ...
                    req.browser_fingerprint = browser_fingerprint;
                    let dec_card_id = enc_dec.cjs_decrypt(
                    ...
                    if (!card_exits)
                        res.status(StatusCode.badRequest).send(
                            ServerResponse.validationResponse("Invalid card id")
                        );
                }
                next();
            }
    ...
    test_capture: async (req, res, next) => {
        try {
            ...
            if (error) {
                res.status(StatusCode.badRequest).send(
                    ServerResponse.errormsg(error.message)
                );
            } else if (check_order.length > 0) {
                next();
            } else {
                res.status(StatusCode.badRequest).send(
                    ServerResponse.validationResponse("Id is not valid")
                );
            }
        } catch (err) {
            res.status(StatusCode.badRequest).send(
    ...
    create_qr_order: async (req, res, next) => {
        ...
        let qr_order_data = await merchantOrderModel.selectMerchantIdByQrCode(record_id);
        // console.log("mode", perDayData.sub_merchant_id, perDayData.currency, mode);

        let mid_data = await helpers.get_mid_by_merchant_id(perDayData.sub_merchant_id, req.body.data.order_details.currency, mode);
        ...
        if (mid_data.length > 0) {
            ...
            console.log("🚀 ~ create_qr_order: ~ order_details.amount:", order_details.amount)
            if ((order_details.amount < min_amount)) {
                return res
                    .status(StatusCode.badRequest)
                    .send(
            ...
            if ((order_details.amount > max_amount)) {
                return res
                    .status(StatusCode.badRequest)
                    .send(
                        ServerResponse.errormsg("Order amount is greater than max order amount")
                    );
            }
        } else {
            return res
                .status(StatusCode.ok)
                .send(ServerResponse.errormsg("Merchant not accepting payments in " +
                    req?.body?.data?.order_details
                        ?.currency +
                    "."));
        }
        ...
        if (perDayData.type_of_qr_code == "Dynamic_QR") {
            ...
            */
            ...
            if (perDayData.qty_frq === "per_day") {
                ...
                var sum_quantity_overall =
                    await qrGenerateModule.per_day_quantity_sum(
                        {
                            'qp.payment_id': `'${perDayData.qr_id}'`,
                            'qp.currency': `'${perDayData.currency}'`,
                        },
            ...
            if (perDayData.qty_frq === "till_expiry") {
                let expiry_date = moment(perDayData.end_date).format(
                    "YYYY-MM-DD"
                );
                var sum_quantity_overall =
            ...
            if (
                parseInt(sum_quantity_overall) + parseInt(req?.body?.data?.order_details
                    ?.quantity) >
            ...
            try {
                ...
                if (result.error) {
                    ...
                    if (invoice_data) {
                        ...
                        // MID Validation
                        let mid_data = await helpers.get_mid_by_merchant_id(
                            invoice_data.sub_merchant_id, invoice_data.currency, invoice_data.mode
                        );
                        ...
                        if (mid_data.length == 0) {
                            mid_error_occurred = true;
                            mid_error = "Merchant not accepting payments in " + invoice_data.currency + ".";
