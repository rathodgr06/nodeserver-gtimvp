            ...
            try {
                ...
                if (result.error) {
                    ...
                    if (check_authtoken) {
                        ...
                        let check_auth_token = await checkCustomToken(
                            authtoken
                        );
                        ...
                        if (check_auth_token.status == "Success") {
                            if (check_auth_token.data.type == "admin") {
                                ...
                                let enc_password = encrypt_decrypt(
                                    "encrypt",
                                    password
                                );
                            ...
                            if (check_auth_token.message) {
                                res.status(StatusCode.ok).send(
                                    ServerResponse.errormsg(
                                        check_auth_token.message
                                    )
                                );
                            } else {
                                res.status(StatusCode.ok).send(
                                    ServerResponse.errormsg(
                                        "Malicious Activity Performed"
                                    )
                                );
                            }
                        }
        ...
        const schema = Joi.object().keys({
            ...
            support_ticket_password: Joi.string()
                ...
                .messages({
                    ...
                    "string.empty":
                        "Support ticket Password should not be an empty",
            ...
            support_ticket: Joi.number()
                ...
                .error(() => {
                    return new Error(
                        "Support ticket should only contain 0 or 1"
                    );
                }),
        });
        ...
        try {
            ...
            if (result.error) {
                ...
                if (!document_exits) {
                    ...
                    if (document_exits) {
                        res.status(StatusCode.ok).send(
                            ServerResponse.errormsg("Document already exist")
                        );
                    }
                }
            }
        ...
        try {
            ...
            if (result.error) {
                ...
                if (record_exits) {
                    ...
                    res.status(StatusCode.ok).send(
                        ServerResponse.errormsg("Record not exist")
                    );
                }
            }
        } catch (error) {
        ...
        try {
            ...
            if (result.error) {
                ...
                if (record_exits) {
                    ...
                    res.status(StatusCode.ok).send(
                        ServerResponse.errormsg(
                            "Record not exist or already activated."
                        )
                    );
                }
            }
            ...
            if (result.error) {
                ...
                if (psp === "Network International") {
                    if (
                        ...
                        "Username not valid/not supplied"
                    ) {
                        message = "Invalid outlet id";
                    } else if (
                        result.error.message ===
                        "Password not valid/not supplied"
                    ) {
        ...
        try {
            ...
            if (result.error) {
                ...
                if (!data_exist) {
                    res.status(StatusCode.ok).send(
                        ServerResponse.errormsg(
                            "Record not exist or already deleted"
                        )
                    );
                } else {
                    next();
    ...
    list_mid: async (req, res, next) => {
        const schema = Joi.object().keys({
            submerchant_id: Joi.string()
                .required()
                .error(() => {
                    return new Error("Invalid Submerchant Id");
                }),
        });
        try {
            const result = schema.validate(req.body);
