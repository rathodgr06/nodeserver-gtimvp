...
async function captureTelr(transactionToCaptureTelr) {
    let captureResponse = [];
    for (let transaction of transactionToCaptureTelr) {
        let temp = await SendToTelr(transaction);
        temp.order_id = transaction.order_id;
        temp.currency = transaction.currency;
        temp.amount = transaction.amount;
        captureResponse.push(await updateTelr(temp))
    }
    return captureResponse;
}
async function captureNI(transactionToCaptureTelr) {
    let captureResponse = [];
    for (let transaction of transactionToCaptureTelr) {
        transaction.order_no = transaction.order_reference_id;
        transaction.payment_no= transaction.payment_id;
        let temp = await SendToNI(transaction);
        captureResponse.push(await updateNI(temp,transaction))
    }
    return captureResponse;
}
async function capturePaytabs(transactionToCapturePaytabs) {
    ...
    for (let transaction of transactionToCapturePaytabs) {
        transaction.order_no = transaction.order_reference_id;
        transaction.payment_no= transaction.payment_id;
        transaction.txn =await helpers.make_sequential_no("TXN");
...
async function SendToNI(_terminalcred) {
    ...
    var support_config = {
        method: "POST",
        url: `${psp_url_credentials.ni.base_url}/transactions/outlets/${_terminalcred.MID}/orders/${_terminalcred.order_no}/payments/${_terminalcred.payment_no}/captures`,
        headers: {
...
module.exports = async () => {
    try{
        ...
        let remainingToCapture = await Model.fetchAll(selection, condition);
        ...
        let [capturedDetails_Telr, captureDatails_NI,capturedDetails_Paytabs] = await Promise.all([captureTelr(transactionToCaptureTelr), captureNI(transactionToCaptureNI),capturePaytabs(transactionToCapturePaytabs)]);
