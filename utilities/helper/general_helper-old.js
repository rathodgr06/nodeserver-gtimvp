...
let helpers = {
    ...
    find_city_by_country: async (name,code) => {
        ...
        let response = await qb
            .select("id")
            .where({ "LOWER(city_name)": name ,"country_code":code,status:0,deleted:0})
    ...
    find_state_id_by_name: async (name,code) => {
        ...
        let response = await qb
            .select("id")
            .where({ "LOWER(state_name)": name ,"country_code":code,status:0,deleted:0})
    ...
    find_city_id_by_name: async (name,code,state) => {
        ...
        let response = await qb
            .select("id")
            .where({ "LOWER(city_name)": name ,"ref_state":state,"country_code":code,status:0,deleted:0})
            .get(config.table_prefix + "city");
        qb.release();
    ...
    get_submerchant_name_by_id: async (id) => {
        ...
        let query = `SELECT ${config.table_prefix + "master_merchant_details"
            }.company_name
                    FROM ${config.table_prefix + "master_merchant"}
                    JOIN ${config.table_prefix + "master_merchant_details"
            } ON ${config.table_prefix + "master_merchant"}.id = ${config.table_prefix + "master_merchant_details"
            }.merchant_id
    ...
    get_captured_date :async (id) => {
        ...
        if (response[0]) {
            return response[0].created_at=="0000-00-00 00:00:00"?'':moment(response[0].created_at).format('DD-MM-YYYY HH:mm:ss');
        } else {
            return "";
        }
    ...
    get_fraud_data: async (order_id,mode) => {
        let qb = await pool.get_connection();
        let response = await qb
            .select("rule_id,order_id,email,mode,rule_if_fail,message,added_date,scenario")
        ...
        if (response[0] && response[0]?.rule_id) {
            let rule_obj = {
                ...
                added_date:moment(response[0].added_date).format('DD-MM-YYYY HH:mm:ss'),
    ...
    get_capture_amount_by_order_id: async (order_id,mode='live') => {
        ...
        let response = await qb
            .select_sum("amount")
            .where({ order_id: order_id,type:['CAPTURE','PARTIALLY_CAPTURE'],is_voided:0 })
            .get(config.table_prefix + txn_table);
        qb.release();
        //console.log(qb.last_query());
    ...
    make_sequential_no: async (pre) => {
        ...
        qb.release();
        //console.log("response[0]?.id "+pre, response[0]?.id);
        return response[0]?.id > 0
            ? parseInt(response[0]?.id) + 1000010001
            : 1000010001;
    },
    ...
    get_mid_by_merchant_id: async (merchant_id, currency,mode) => {
        ...
        let response = await qb
            ...
            .join(config.table_prefix + "master_currency c", "m.currency_id=c.id","inner")
            .like("m.class", "ecom")
            .where(condition)
            .get();
        qb.release();
    ...
    checkCardExistByCardNoAndCID: async (condition, card_no) => {
        ...
        if (response.length > 0) {
            ...
            let card_number_arr = await helpers.keyByArr(
                response,
                "card_number"
            );
            ...
            let cipher_keys = await helpers.get_cipher_keys_by_ids(
                cipher_keys_ids
            );
            ...
            for (let i = 0; i < card_number_arr.length; i++) {
                let private_key =
                    cipher_keys[
                        card_number_cipher[card_number_arr[i]].cipher_id
                    ].private_key;
                let private_iv =
                    cipher_keys[
                        card_number_cipher[card_number_arr[i]].cipher_id
    ...
    LastSubscriberDate: async (plan_id) => {
        ...
        let response = await qb
            ...
            .get();
        qb.release();
        return response[0]?.added_date ? moment(response[0]?.added_date).format('DD-MM-YYYY HH:mm:ss') : '';
    },
    ...
    get_recurring_start_next_date: async (condition_obj, order) => {
        ...
        response = await qb.query(
            ...
            config.table_prefix + "subscription_recurring" +
            " where " +
            condition + "order by id " + order + " LIMIT 1"
        );
        qb.release();
        return response[0];
    },
    get_recurring_last_status: async (condition_obj) => {
        ...
        response = await qb.query(
            ...
            config.table_prefix + "subscription_recurring" +
            " where " +
            condition + " and ( is_paid=1 or is_failed=1 ) order by id desc limit 1"
        );
    ...
    check_if_currency_value_exist: async (psp, currency, table) => {
        ...
        let qb = await pool.get_connection();
        let query =
            `select * from ${config.table_prefix + table
            } where psp = ${psp} and deleted = 0 and (` + like_str + `)`;
        let response = await qb.query(query);
        qb.release();
        if (response.length > 0) {
            return true;
    ...
    check_if_mcc_value_exist: async (psp, mcc, table, master_id) => {
        ...
        let query =
            `select * from ${config.table_prefix + table
            } where psp = ${psp} and deleted = 0 ` + master_con + like_str + ``;
        let response = await qb.query(query);
    ...
    check_buy_rate: async (master_id = null, req, currency = null, mcc=null, table) => {
        ...
        let mcc_str = '';
        if (currency) {
            const currencyConditions = currency.split(",").map((currency) => `currency LIKE '%${currency}%'`);
            const currencyQuery = currencyConditions.join(" OR ");
            like_str = `AND (${currencyQuery})`;
        }
    ...
    check_if_currency_value_exist_pricing: async (
        currency,
        table,
        plan_name
    ) => {
    ...
    common_delete: async (condition, table) => {
        let qb = await pool.get_connection();
        let response = await qb.delete(config.table_prefix + table, condition);
        qb.release();
        return response;
    },
        ...
        if (mode === "test") {
            query = `SELECT
                    ...
                    (
                        ...
                        pg_test_order_txn
                        WHERE
                        order_id = ${order_id} and status!='AWAIT_3DS' and status='AUTHORISED'
    ...
    get_capture_date_by_order_id: async (order_id, table) => {
       ...
       let tname= config.table_prefix +table;
        let query = "SELECT `created_at` FROM `" + tname + "` WHERE `order_id` = '" + order_id +"' AND `status` = 'AUTHORISED' AND (`type` = 'CAPTURE' or type='PARTIALLY_CAPTURE') ";
        let response = await qb.query(query);
        qb.release();
        if (response[0]) {
            return moment(response[0].created_at).format("DD-MM-YYYY H:mm:ss");
        ...
        let qb = await pool.get_connection();
        let tname = config.table_prefix + table;
        let query = "SELECT `created_at` FROM `" + tname + "` WHERE `order_id` = '" + order_id + "' AND `status` = 'AUTHORISED' AND (`type` = 'REFUND' or type='PARTIALLY_REFUND') ";
        let response = await qb.query(query);
        qb.release();
        if (response[0]) {
            return moment(response[0].created_at).format("DD-MM-YYYY H:mm:ss");
    ...
    get_partial_refund_date_by_order_id: async (order_id, table) => {
        ...
        let qb = await pool.get_connection();
        let tname = config.table_prefix + table;
        let query = "SELECT `created_at` FROM `" + tname + "` WHERE `order_id` = '" + order_id + "' AND `status` = 'AUTHORISED' AND  type='PARTIALLY_REFUND' ";
        let response = await qb.query(query);
        qb.release();
    ...
    get_txn_details_by_order_status: async (condition, data, table) => {
        ...
        if (response[0]) {
            if (data == "CAPTURE") {
                ...
                return (
                    response[0]?.amount.toFixed(2) + " " + response[0]?.currency
                );
            }
        } else {
            return "-";
...
WHERE CONCAT(SUBSTRING(o.expiry, 1, 4), '-', SUBSTRING(o.expiry, 6, 2))
    ...
    calculateDate: async (nextDueDate, payment_interval, paymentFrequencyType) => {
    ...
    calculatePreDate: async (nextDueDate, payment_interval, paymentFrequencyType) => {
    ...
    getCurrency : async (key, value) => {
        ...
        for (const currency_id of valueArray) {
            let id = await enc_dec.cjs_decrypt(currency_id);
    ...
    getPaymentMode: async (key, value) => {
        ...
        if (response && response.length > 0) {
            var new_str = response[0]?._str=="APPLE PAY"?response[0]?._str.split(' ').join('_'):response[0]?._str;
    ...
    getCardSchema: async (key, value) => {
        ...
        const ids = [];
        for (const id of valueArray) {
            let scheme = await enc_dec.cjs_decrypt(id);
            ids.push(scheme);
        }
        ...
        let qb = await pool.get_connection();
            response = await qb.query(
                "select country_code from " +
                    config.table_prefix +
                    table_name + " where iso2 = '" + code + "' order by id asc limit 1" 
         ...
         for(let item of myArray){
           ...
           new_array.push(temp)
            ...
            for (var i = 0; i < new_array.length; i++) 
            {
                ...
                {            
                        if (new_array[i].dom_int == new_array[j].dom_int && new_array[i].payment_methods == new_array[j].payment_methods && new_array[i].payment_schemes == new_array[j].payment_schemes && new_array[i].currency == new_array[j].currency) 
                        {
   ...
   invoice_shared: async (invoice_id, type) => {
    ...
    addTransactionFailedLogs:async(data)=>{
        let query = await helpers.buildInsertQuery(config.table_prefix+'order_life_cycle_logs',data)
        let qb = await pool.get_connection();
        let response = await qb.query(query);
        //let response = await qb.returning("id").insert(config.table_prefix+'order_life_cycle_logs', data);
        qb.release();
        return response;
    },
    addTempCard:async(data)=>{
        let query = await helpers.buildInsertQuery(config.table_prefix+'temp_cards',data)
        let qb = await pool.get_connection();
        let response = await qb.query(query);
        qb.release();
        return response;
    ...
    fetchTempLastCard:async(condition)=>{
        let qb = await pool.get_connection();
        let response  = await qb.select('*').from(config.table_prefix+'temp_cards').where(condition).order_by('id','desc').limit(1).get();
        //console.log(qb.last_query());
        // let response1 = await qb.delete(config.table_prefix+'temp_cards',condition);
        qb.release();
        // console.log(qb.last_query());
        return response?.[0];
    },
    fetchLastTryData:async(condition)=>{
        let qb = await pool.get_connection();
        let response  = await qb.select('card_proxy,card,cipher_id,card_holder_name,expiry,status_code,3ds_version').from(config.table_prefix+'order_life_cycle_logs').where(condition).order_by('id','desc').limit(1).get();
        qb.release();
        //.log(qb.last_query());
        return response?.[0];
    },
    fetchPSPBYTXN:async(condition)=>{
        let qb = await pool.get_connection();
        let response  = await qb.select('psp').from(config.table_prefix+'order_life_cycle_logs').where(condition).order_by('id','desc').limit(1).get();
        qb.release();
        //console.log(qb.last_query());
        return response?.[0];
    },
...
rule_string = rule_string.replace(/\n|<br\s*\/?>/gi, "\r");
    ...
    getRoutingRuleDetails:async(order_id,mode)=>{
        let qb = await pool.get_connection();
        let response = await qb.select('rule_id,mid_list').from(config.table_prefix+'order_life_cycle').where({order_id:order_id,mode:mode}).get();
        
        if(response && response?.length>0){
            if (response[0].rule_id>0) {
                let response1 = await qb.select('rule_name').from(config.table_prefix + 'routing_rule').where({ id: response[0].rule_id }).get();
                qb.release();
    ...
    getRetryAndCascade:async(order_id)=>{
        let qb= await pool.get_connection();
        let response = await qb.select('retry,cascade,mid_list').from(config.table_prefix+'order_life_cycle').where({order_id:order_id}).get();
        qb.release();
        if(response && response?.length>0){
            return response[0];
        }else{
    ...
    extractAllText: async(str) => {
      ...
      checkOrderWasRejected:async(condition)=>{
        let qb= await pool.get_connection();
        let response = await qb.select('id').from(config.table_prefix+'order_life_cycle_logs').where(condition).order_by('id','desc').limit(1).get();
        qb.release();
        if(response && response?.length>0){
            return true;
        }else{
    ...
    orderCycle: async (condition) => {
        let qb = await pool.get_connection();
        let response = await qb.select('*').from(config.table_prefix + 'order_life_cycle').where(condition).order_by('id', 'desc').limit(1).get();
        qb.release();
        return response[0];
    },
    lastCardUsed: async (condition) => {
        let qb = await pool.get_connection();
        let response = await qb.select('*').from(config.table_prefix + 'order_life_cycle_logs').where(condition).order_by('id', 'desc').limit(1).get();
        qb.release();
         return response[0];
    },
    lastTwoCardUsed: async (condition) => {
        let qb = await pool.get_connection();
        let response = await qb.select('*').from(config.table_prefix + 'order_life_cycle_logs').where(condition).order_by('id', 'desc').limit(2).get();
        qb.release();
       // console.log(qb.last_query());
        return response;
    },
    midDetails: async (order_id,txn_id,mode) => {
        let sql = `SELECT m.label,m.terminal_id,o.psp from pg_mid m join pg_order_life_cycle_logs o on o.terminal=m.terminal_id where o.txn='` + txn_id + `' and o.mode='` + mode + `' and o.order_id='` + order_id +`' `;
        let qb = await pool.get_connection();
        let response = await qb.query(sql);
        qb.release();
        return response[0];
      ...
      checkForHardOrSoftDeclined:async(condition)=>{
        let qb= await pool.get_connection();
        let response = await qb.select('psp,status_code').from(config.table_prefix+'order_life_cycle_logs').where(condition).order_by('id','desc').limit(1).get();
       // console.log(qb.last_query());
        if(response && response.length>0){
            let response_code_details = await qb.select('soft_hard_decline').from(config.table_prefix+'response_code').where({psp_name:response[0].psp,response_code:response[0].status_code}).limit(1).get();
            // console.log(qb.last_query());
            // console.log(`response code details`);
            // console.log(response_code_details);
            qb.release();
            if(response_code_details && response_code_details?.length>0 && response_code_details[0]?.soft_hard_decline.toUpperCase()=='HARD'){
                return true;
            }else{
                return false;
            }
      ...
      fetch3dsVersion:async(condition)=>{
        let qb= await pool.get_connection();
        let response = await qb.select('3ds_version').from(config.table_prefix+'order_life_cycle_logs').where(condition).order_by('id','desc').limit(1).get();
        //console.log(qb.last_query());
        qb.release();
        let obj = {
            result:false,
    ...
    updateOrderCycle:async(order_id,mode) => {
          let orderCycle = await helpers.orderCycle({ order_id: order_id, mode: mode });
        ...
        if (orderCycle.original_mid_list == "" || orderCycle.original_mid_list == null){
