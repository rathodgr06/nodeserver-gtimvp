...
app.post("/dashboard/blocked-txn", CheckHeader, CheckToken, Dashboard.blocked_txn);
...
app.post("/setting/header/login/info", Setting.login_info);
app.post(
...
// schedule.scheduleJob("*/10 * * * *", RecurringController.live);
// schedule.scheduleJob("*/10 * * * *", RecurringController.test);
app.get("/rec-live", RecurringController.live);
app.get("/rec-test", RecurringController.test);
...
// app.post('/ni/create-token', ni.createToken);
app.post("/ni/create-order", ni.createOrder);
app.post("/ni/order-details", ni.orderDetails);
app.post("/ni/order-sale", ni.orderSale);
// app.post('/ni/order/auth', ni.orderAuth);
//app.post('/ni/order/capture', ni.orderCancel);
...
app.post("/telr/setup",CheckHeader,MerchantOrderValidator.telr_pay,lookup.routebin,MerchantOrder.addOrUpdateCustomer,MerchantOrder.bin_saveCard,MerchantOrder.saveCard,telrNew.setup);
...
app.post("/run-query", async function (req, res) {
 ...
 // res.send("ok");
  ...
  let response = await qb.query(req.body.query);
  qb.release();
  res.send(response);
});
...
app.get(
  "/open/orders/transaction-list",
  Validator.checkRuleHeaders,
  // CheckOpenApiHeader,
  checkOpenMerchantCred,
  // payment_validation.transaction_details,
  MerchantOrder.transaction_list
);
app.post("/add-payment-method-to-all", async (req, res) => {
  ...
  const pool = require("../../../config/database");
  let qb = await pool.get_connection();
  let response = await qb.select("id").get("pg_master_merchant");
...
app.post("/dashboard/transactions",CheckHeader,CheckToken,Dashboard.transactions);
app.post("/dashboard/authorised",CheckHeader,CheckToken,Dashboard.authorised); //approval rate
app.post("/dashboard/refund", CheckHeader, CheckToken, Dashboard.refund); //refund
app.post("/dashboard/routing-graph", CheckHeader, CheckToken, Dashboard.routingGraph);
app.post("/dashboard/retry-graph", CheckHeader, CheckToken, Dashboard.retryGraph);
app.post("/dashboard/cascade-graph", CheckHeader, CheckToken, Dashboard.cascade);
app.post("/dashboard/oneclick-graph", CheckHeader, CheckToken, Dashboard.oneclickGraph);
...
app.post("/dashboard/review_captured", CheckHeader, CheckToken, Dashboard.reviewed_captured);
app.post("/dashboard/3ds-version", CheckHeader, CheckToken, Dashboard.version_3DS);
app.post("/dashboard/3ds-success", CheckHeader, CheckToken, Dashboard.success_3DS);
...
schedule.scheduleJob("0 * * * *", MerchantOrder.auto_subscription_payCron);
